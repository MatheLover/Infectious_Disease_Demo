    ...
    if request.GET.get("GraphType") == "Scatterplot":
        ...
        if obtained_country == "World":
            for m in Malaria.objects.all():
                ...
                slider_lon_list.append(m.Longitude)
                slider_name_list.append(m.Country)
                slider_gdp_list.append(m.GDP_per_capita)
                year = str(m.Year)
                if year != '2010':
                    year = year + '/12/31'
                slider_year_list.append(year)
        ...
        country_case = zip(slider_name_list, slider_gdp_list, slider_year_list)
        slider_zipped_country_case = list(country_case)
        df_slider = pd.DataFrame(data=slider_zipped_country_case, columns=['Country', 'GDP_per_capita', 'Year'])
        df_slider = df_slider[df_slider.GDP_per_capita != 0]
        ...
        # Construct color map
        max_color = max(combined_df['GDP_per_capita'])
        min_color = min(combined_df['GDP_per_capita'])
        color_map = cm.linear.YlOrRd_09.scale(min_color, max_color)
        combined_df['color'] = combined_df['GDP_per_capita'].map(color_map)
        # print(combined_df)
        ...
        _ = color_map.add_to(m)
        color_map.caption = "GDP per capita in " + obtained_country

        m.save("malaria/malaria_gdp_per_capita.html")
        m = m._repr_html_()
        context = {
            'm': m
        }
